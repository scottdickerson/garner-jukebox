export const VIDEOS = 'videos'

export const imageAndVideoContent = [
    {
        id: '1930s',
        heading: { en: '1930s', es: 'La década de 1930' },
        title: { en: 'The Party Begins', es: 'Comienza la fiesta' },
        content: [
            '/images/1930s/1930s-1.png',
            '/images/1930s/1930s-2.png',
            '/images/1930s/1930s-3.png',
            '/images/1930s/1930s-4.png',
            '/images/1930s/1930s-5.png',
        ],
        captions: [
            {
                en: 'Even before the dance pavilion existed, people were dancing under the stars at Garner.',
                es: 'Incluso antes de que existiera el pabellón de baile, la gente bailaba bajo las estrellas en Garner.',
            },
            {
                en: 'There was no jukebox, so all music would have been played by live bands.',
                es: 'No había rocola, así que toda la música la tocaban bandas en vivo.',
            },
            {
                en: 'Some CCC enrollees recall a performance by <span class="underline">Bob Wills & His Texas Playboys.</span>',
                es: 'Algunos miembros del CCC recuerdan una interpretación de <span class="underline">Bob Wills & His Texas Playboys.</span>',
            },
            {
                en: 'Mexican-American enrollees invited local conjunto and Tejano performers, whose names never got recorded. They may have sounded a bit like Lydia Mendoza…',
                es: 'Algunos miembros mexicano-estadounidenses invitaban a músicos tejanos y conjuntos locales, cuyos nombres nunca fueron registrados. Deben haber sonado un poco como Lydia Mendoza…',
            },
            {
                en: 'Popular dances of the era included line dances, the Lindy Hop, and the Texas Tommie (which originated in San Francisco).',
                es: 'Los bailes populares de la época incluían las danzas en línea, el Lindy Hop y el Texas Tommie (originario de San Francisco).',
            },
        ],
    },
    {
        id: '1940s',
        heading: { en: '1940s', es: 'La década de 1940' },
        title: { en: 'The Dance Floor', es: 'La pista de baile' },
        content: [
            '/images/1940s/1940s-1.png',
            '/images/1940s/1940s-2.png',
            '/images/1940s/1940s-3.png',
            '/images/1940s/1940s-4.png',
            '/images/1940s/1940s-5.png',
            '/images/1940s/1940s-6.png',
        ],
        captions: [
            { en: 'Garner opened to the public in 1941.', es: '' },
            { en: 'The first jukebox arrived in this decade.', es: '' },
            {
                en: 'One record that surely spun often on this early music machine was Gene Autry’s version of “Blueberry Hill.”',
                es: 'Una grabación que seguramente sonaba con frecuencia en esta temprana máquina de música era la versión de “Blueberry Hill” de Gene Autry.',
            },
            {
                en: 'Many of the CCC enrollees who built the park went off to fight in WWII (1941-1945). In later years, many agreed that their experiences here prepared them for war.',
                es: 'Muchos de los miembros del CCC que construyeron el parque fueron a combatir en la Segunda Guerra Mundial (1941-1945). Años más tarde, muchos coincidieron en que sus experiencias aquí los prepararon para la guerra.',
            },
            {
                en: 'Another popular tune for dancers was “Tuxedo Junction” by Glenn Miller.',
                es: 'Otra melodía popular para bailar era “Tuxedo Junction” de Glenn Miller.',
            },
            {
                en: 'Jitterbug dances were popular across the nation. ',
                es: 'Los bailes de jitterbug eran populares en el país.',
            },
        ],
    },
    {
        id: '1950s',
        heading: { en: '1950s', es: 'La década de 1950' },
        title: { en: 'The Bop Era', es: 'La Era del Bop' },
        content: [
            '/images/1950s/1950s-1.png',
            '/images/1950s/1950s-2.png',
            '/images/1950s/1950s-3.png',
            '/images/1950s/1950s-4.png',
            '/images/1950s/1950s-5.png',
        ],
        captions: [
            {
                en: 'Summer nights during the 1950s saw the twirls of full-skirted petticoats under the stars in the pavilion.',
                es: 'Las noches de verano en la década de 1950 fueron testigo de los giros de las largas faldas con enaguas bajo las estrellas en el pabellón.',
            },
            {
                en: 'By this decade, many families had already started coming back to Garner year after year.',
                es: 'Para esta década, muchas familias ya habían comenzado a regresar a Garner año tras año.',
            },
            {
                en: 'Big hits on the jukebox included the first appearance of Frankie Miller’s “Blackland Farmer” (which has remained on the playlist well into the 2020s).',
                es: 'Los grandes éxitos en la rocola incluían la primera aparición de “Blackland Farmer” de Frankie Miller (que formó parte de la playlist hasta bien entrada la década de 2020).',
            },
            {
                en: 'During the 1950s, television spread trends in music and dance like never before. ',
                es: 'Durante la década de 1950, la televisión difundió tendencias en música y baile como nunca antes.',
            },
            {
                en: 'Many dances got lumped together into a single category called the Bop.',
                es: 'Muchos bailes fueron agrupados en una sola categoría llamada Bop.',
            },
        ],
    },
    {
        id: '1960s',
        heading: { en: '1960s', es: 'La década de 1960' },
        title: { en: 'Whipping it up', es: 'A sacudirse' },
        content: [
            '/images/1960s/1960s-1.png',
            '/images/1960s/1960s-2.png',
            '/images/1960s/1960s-3.png',
            '/images/1960s/1960s-4.png',
            '/images/1960s/1960s-5.png',
            '/images/1960s/1960s-6.png',
        ],
        captions: [
            {
                en: 'Some say the 1960s were the heyday of dancing at Garner. ',
                es: 'Algunos dicen que la década de 1960 fue el apogeo del baile en Garner.',
            },
            {
                en: 'The soundtrack started to sound a little more rock n’ roll. ',
                es: "La banda sonora empezó a sonar un poco más como <span>rock 'n' roll.</span>",
            },
            {
                en: 'A Houston-based band called the Triumphs released a hit record called “Garner State Park,” rocked the airwaves across Texas. ',
                es: 'Una banda de Houston llamada The Triumphs lanzó un exitoso disco llamado “Garner State Park” que sonó a más no poder en las radios de Texas.',
            },
            {
                en: 'Dancers coveted the “slick slab,” the best dancing spot on the pavilion.',
                es: 'Todos deseaban bailar en el “sector principal”, el mejor lugar de la pista de baile en el pabellón.',
            },
            {
                en: 'Songs like Dale Hawkins’ “Susie-Q” sent dancers to the slick slab over and over again.',
                es: 'Temas musicales como “Susie-Q” de Dale Hawkins hacían que la gente regresara a bailar una y otra vez.',
            },
            {
                en: 'During the 1960s, a new dance craze called the Garner Whip emerged right here in the park!',
                es: 'Durante la década de 1960, un nuevo baile de moda llamado Garner Whip surgió aquí mismo en el parque.',
            },
        ],
    },
    {
        id: '1970s',
        heading: { en: '1970s', es: 'La década de 1970' },
        title: { en: 'A Legacy of Fun', es: 'un legado de diversión' },
        content: [
            '/images/1970s/1970s-1.png',
            '/images/1970s/1970s-2.png',
            '/images/1970s/1970s-3.png',
            '/images/1970s/1970s-4.png',
        ],
        captions: [
            {
                en: 'By the 1970s, the dance floor at Garner had become an institution. ',
                es: 'Hacia la década de 1970, la pista de baile en Garner se había vuelto una institución.',
            },
            {
                en: 'But there was still room for new stories to emerge.',
                es: 'Pero aún había margen para que emergieran nuevas historias.',
            },
            {
                en: 'Johnny Rodriguez,got his start right here at Garner. He returned often to play his hits, including “Ridin’ My Thumb to Mexico.”',
                es: 'Johnny Rodriguez empezó justo aquí, en Garner. Regresó a menudo para tocar sus éxitos, entre ellos “Ridin’ My Thumb to Mexico”.',
            },
            {
                en: 'All parties have to come to an end, and when the dance floor closed in the 1970s, it was usually a slow dance to “The Last Song” by Gene Thomas.',
                es: 'Todas las fiestas deben terminar y, cuando la pista de baile cerraba en la década de 1970, habitualmente era con el tema lento “The Last Song”, de Gene Thomas.',
            },
        ],
    },
    {
        id: VIDEOS,
        heading: { en: 'Videos', es: 'Vídeos' },
        title: { en: '', es: '' },
        content: [
            {
                en: '/images/videos/ExploreTheseVideos.png',
                es: '/images/videos/ExploreTheseVideosEs.png',
            },
            '/videos/Navarro Home Videos/1963 Swimming at Garner State Park.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 1.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 2.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 3.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 4.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 5.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 6.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 7.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 8.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 9.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 10.mp4',
            '/videos/Navarro Home Videos/1963 Trip to Garner State Park Part 11.mp4',
            '/videos/Navarro Home Videos/1964 Garner State Park.mp4',
            '/videos/Dance Videos/DontRockTheJukebox.mp4',
            '/videos/Dance Videos/BabyBlue.mp4',
            '/videos/Dance Videos/RockinRobin.mp4',
            '/videos/Dance Videos/JukeboxShots.mp4',
        ],
        captions: [
            {
                en: '',
                es: '',
            },
            {
                en: 'Swimming at Garner State Park<br/>Texas 1963',
                es: 'Nadar en el Parque Estatal Garner<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
1: Leaving Houston<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>1: Salir de Houston<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
2: Arriving at the park</br>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>2: Llegar al parque<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
3: Getting snacks and heading to the water<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>3: Conseguir snacks e ir al agua<br>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
4: Cooking and swimming<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>4: Cocinar y nadar<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
5: Squirrels, swimming and horses</br>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>5: Ardillas, natación y caballos<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
6: Riding horses & dancing teenagers<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>6: Cabalgatas y adolescentes bailando<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
7: Setting up camp<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>7: Armar el campamento<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
8: Scenes around the camp site<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>8: Escenas alrededor del área de camping<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
9: On the river and at the pavilion<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>9: En el río y en el pabellón<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
10: Swimming and miniature golf<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>10: Natación y minigolf<br/>Texas 1963',
            },
            {
                en: `Trip to Garner State Park<br/>
11: Around the camp and on the river<br/>
Texas 1963`,
                es: 'Viaje al Parque Estatal Garner<br/>11: Alrededor del campamento y en el río<br/>Texas 1963',
            },
            {
                en: `Garner State Park<br/>1964`,
                es: 'Parque Estatal Garner<br/>1964',
            },
            {
                en: `Garner State Park Dance Floor<br/>
“Don’t Rock the Jukebox”`,
                es: 'Pista de baile del Parque Estatal Garner<br/>“Don’t Rock the Jukebox”',
            },
            {
                en: `Garner State Park Dance Floor<br/>
“Baby Blue”`,
                es: 'Pista de baile del Parque Estatal Garner<br/>“Baby Blue”',
            },
            {
                en: `Garner State Park Dance Floor<br/>
“Rockin’ Robin”`,
                es: 'Pista de baile del Parque Estatal Garner<br/>“Rockin’ Robin”',
            },
            {
                en: `Garner State Park Dance Floor<br/>
Jukebox Shots`,
                es: 'Pista de baile del Parque Estatal Garner<br/>Jukebox Shots',
            },
        ],
    },
]
